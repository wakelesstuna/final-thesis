...
import io.wakelesstuna.user.generated.client.PostRepresentation;
...
class PostDataFetcherTest {
    ...
    UUID postId = UUID.fromString("d597ea0a-a541-4134-b4d5-e9da4cf3e96f");
    ...
    @Test
    void usernameShouldBeReturned() {
        var expected = "testUser";
        EntitiesGraphQLQuery entitiesQuery = new EntitiesGraphQLQuery.Builder()
                .addRepresentationAsVariable(PostRepresentation.newBuilder()
                        .id(postId).userId(userId).build()).build();

        GraphQLQueryRequest request = new GraphQLQueryRequest(
                entitiesQuery,
                new EntitiesProjectionRoot().onPost().id().userId().user().username(),
                scalars
        );
        ...
        Post post = response.extractValueAsObject("data._entities[0]", Post.class);

        assertEquals(expected, post.getUser().getUsername());
    }
